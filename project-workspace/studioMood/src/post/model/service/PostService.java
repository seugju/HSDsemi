...
public class PostService {
	...
	public PostPageData searchKeyword(int reqPage, String keyword, String type) {
		Connection conn = JDBCTemplate.getConnection();
		...
		if(type.equals("postTitle")) {
			...
			if(pageNo != 1) {
				pageNavi += "<a class='btn' href='/searchKeyword?reqPage="+(pageNo-1)+"&type=postTitle&keyword="+keyword+"'>이전</a>";
			}
			for(int i=0; i<pageNaviSize;i++) {
				if(reqPage == pageNo) {
					pageNavi += "<span class='selectPage'>"+pageNo+"</spna>";
				}else {
					pageNavi +="<a class='btn' href='/searchKeyword?reqPage="+pageNo+"&type=postTitle&keyword="+keyword+"'>"+pageNo+"</a>";
				}
				pageNo++;
				if(pageNo > totalPage) {
					break;
			...
			if(pageNo <= totalPage) {
				pageNavi += "<a class='btn' href='/searchKeyword?reqPage="+pageNo+"&type=postTitle&keyword="+keyword+"'>다음</a>";
			}
			...
			if(pageNo != 1) {
				pageNavi += "<a class='btn' href='/searchKeyword?reqPage="+(pageNo-1)+"&type=postWriter&keyword="+keyword+"'>이전</a>";
			}
			for(int i=0; i<pageNaviSize;i++) {
				if(reqPage == pageNo) {
					pageNavi += "<span class='selectPage'>"+pageNo+"</spna>";
				}else {
					pageNavi +="<a class='btn' href='/searchKeyword?reqPage="+pageNo+"&type=postWriter&keyword="+keyword+"'>"+pageNo+"</a>";
				}
				pageNo++;
				if(pageNo > totalPage) {
					break;
			...
			if(pageNo <= totalPage) {
				pageNavi += "<a class='btn' href='/searchKeyword?reqPage="+pageNo+"&type=postWriter&keyword="+keyword+"'>다음</a>";
			}
	...
	public PostPageData searchTitle(int reqPage, String keyword) {
		Connection conn = JDBCTemplate.getConnection();
		PostDao dao = new PostDao();
		...
		PostPageData ppd1 = new PostPageData(list, pageNavi);
		JDBCTemplate.close(conn);
		return ppd1;
	}

	public PostPageData searchWriter(int reqPage, String keyword) {
		Connection conn = JDBCTemplate.getConnection();
		PostDao dao = new PostDao();
		int totalCount = dao.searchWriterCount(conn, keyword);
		int numPerPage = 10;
		int totalPage = 0;
		if(totalCount%numPerPage == 0) {
			totalPage = totalCount / numPerPage;
		...
		PostPageData ppd2 = new PostPageData(list, pageNavi);
		JDBCTemplate.close(conn);
		return ppd2;
	}
